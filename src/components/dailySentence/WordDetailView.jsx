			...
			<div className="word_detail_wrap">
				<div className="word_detail_cont">
					<div className="word_detail_top">
						<div className="daily_sentence_view_date">{wordList?.createTime}</div>
						<button className={wordList?.memorization === "Y"? "word_card_check on" : "word_card_check"}>
					...
					<div className="word_detail">
						<div className="word_card_name word_detail_name">{wordList?.word}</div>
						<div className="word_card_read">{wordList?.read}</div>
						<button className="word_card_headset word_detail_headset">
							<i ref={headSetRef} className="xi-headset" onClick={headSetBtnOnClick(wordList?.soundPath)}></i>
						</button>
						<div className="word_card_mean_wrap word_detail_mean_wrap">
							{
								wordList?.mean?.split(",")?.map((value, idx) =>
										<div key={idx} className="word_card_mean_list">{idx + 1}.{value}</div>
								)
              }
						</div>
						<WordDetailList details={wordList?.details}></WordDetailList>
					</div>
				</div>
			</div>
		</>
