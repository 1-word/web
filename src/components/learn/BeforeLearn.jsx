...
function BeforeLearn({ deleteModalAfterTime, learnType }) {
  ...
  const handleSelectItems = (refIdx, id) => (e) => {
    ...
    if (name === "memorization") {
      ...
      const count =
        value === "Y"
          ? quizInfo.current.memorizedCount
          : value === "N"
          ? quizInfo.current.unMemorizedCount
          : quizInfo.current.totalCount;
      // 슬라이드 반영
      setSlider({
        ...slider,
        persent: 100,
  ...
  // 슬라이드
	const calculateSliderStyle = (newValue) => {
		const currentSlider = sliderRef.current;
		const percentage =
			(100 * (newValue - currentSlider.min)) /
			(currentSlider.max - currentSlider.min);
		return `linear-gradient(90deg, ${slider.fill} ${percentage}%, ${
			slider.bg
		} ${percentage + 0.1}%)`;
	};
	...
	const updateStep = (newValue) => {
		...
		setSlider({
			...slider,
			persent: (100 * (newValue - currentSlider.min)) / (currentSlider.max - currentSlider.min),
			value: newValue,
			style: calculateSliderStyle(newValue),
		});
	
		// 슬라이더 버튼 활성화/비활성화 설정
		setDisabled({
			prev: newValue <= currentSlider.min,
			next: newValue >= currentSlider.max,
		});
	};
	

  const handleStepper = (step) => {
    const newValue = Number(slider.value) + step;
    updateStep(newValue);
  };
      ...
      <div className="before_learn_wrap">
        <div className="before_learn_choose">
          ...
          <div className="before_learn_contents">
            <button className="btn-light sizeM" onClick={handleSelectVocabook}>
              {folderInfo.name}
            </button>
          </div>
        </div>
        {learnType === "quiz" && (
          <div className="before_learn_choose">
            ...
            <ul
              ...
              className="before_learn_contents before_learn_select"
            >
              <li
                data-value="word"
                className="before_learn_list on"
                onClick={handleSelectItems(0, 0)}
              >
                단어
                <svg
              ...
              <li
                data-value="mean"
                className="before_learn_list"
                onClick={handleSelectItems(0, 1)}
              >
                뜻
                <svg
                  className="check_icon"
        ...
        <div className="before_learn_choose">
          <span className="before_learn_title">
            ...
            <span className="before_learn_stepper">
              <button
                className="before_learn_stepper_btn"
                disabled={disabled.prev}
                onClick={() => handleStepper(-1)}
              >
                <i className="xi-angle-left" />
              </button>
              <span className="before_learn_stepper_length">
                {slider.value}
              </span>
              <button
                className="before_learn_stepper_btn"
          ...
          <div className="before_learn_contents">
            <input
              ref={sliderRef}
              style={{ background: slider.style }}
              className="before_learn_range"
              type="range"
              min={1}
              max={slider.max}
              value={slider.value}
              onInput={(e) => updateStep(Number(e.target.value))}
            />
          </div>
        </div>
        <div className="modal_full_btn_wrap">
          <button className="btn-fill sizeL" onClick={handleSubmit}>
            학습 시작
          </button>
        </div>
      </div>
