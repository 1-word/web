    ...
    <>
      {
        shareWordBook?.data.map((item, idx) => (
          ...
          <div className="lounge-list-title">
            <div className="lounge-list-title-dot" style={{ backgroundColor: item?.background || "#946CF4" }}></div>
            
            <p>{item?.name}</p>
          </div>
          <p className="lounge-list-title-sub">{item?.nickname} 님의 단어장</p>
          <p className="lounge-list-title-sub">총 단어 : {item?.totalCount}개</p>
          </div>
        </li>
        ))
      }
